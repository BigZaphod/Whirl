...
%           Replaced all occurrences of takeout/3 with delete/3.
...
:- ensure_loaded([library(math), library(lists)]).
...
%    (not in any respect resembling a memory leak ... *cough*).
  ...
  floor(X, Y),
  NewIdx is Y + Idx,
  delete(Memory, cell(NewIdx, SomeNewV), SomeMem),
  (SomeMem = Memory ->
     NewV = value(0),
     Mem = Memory
   ;
